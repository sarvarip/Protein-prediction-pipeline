...
import featurex_pipeline_aac as FX
import modelbuilder_adenolim as trainer
...
vector_name = "pipeline_complete_antibac_aac_nobatch"
    ...
    for s in seqs:
        ...
        if inputfile not in negfile: #because if it is valid sequence test has already been done,do not do it twice, purely for efficiency
            if not all((c in chars) for c in s) or len(s) < (minlength +1):
                dropped = dropped + 1
                continue
        print("Extracting descriptors for sequence: " + str(current_seq) + "/" + str(total_samples))
        s_x_seq = time.time()
